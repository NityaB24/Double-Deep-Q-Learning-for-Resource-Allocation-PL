...
class V2Vchannels:
    ...
    def __init__(self, n_Veh, n_RB, velocities, communication_range):
        self.t = 1
        self.h_bs = 1.5  # Base station height in meters
        self.h_ms = 1.5  # Mobile station height in meters
        self.fc = 2  # Carrier frequency in GHz (e.g., 2 GHz)
        self.decorrelation_distance = 10  # Decorrelation distance in meters
        self.shadow_std = 3  # Shadow fading standard deviation
        self.n_Veh = n_Veh  # Number of vehicles
        self.n_RB = n_RB  # Number of resource blocks
        self.velocities = velocities  # List of vehicle velocities
        self.communication_range = communication_range  # Communication range in meters
        self.update_shadow([])  # Initialize shadow fading
        self.update_positions([])  # Initialize vehicle positions
        self.update_fast_fading()  # Initialize fast fading
    ...
    def update_pathloss(self):
        ...
        self.LinkReliability = np.zeros((len(self.positions), len(self.positions)))  # Reliability matrix
        
        for i in range(self.n_Veh):  # Ensure that the range is from 0 to self.n_Veh - 1
            for j in range(self.n_Veh):  # Same for j
                self.PathLoss[i][j] = self.get_path_loss(self.positions[i], self.positions[j])

                # Ensure velocities array is the correct size
                if i < len(self.velocities) and j < len(self.velocities):
                    vij = abs(self.velocities[i] - self.velocities[j])  # Relative velocity
                else:
                    vij = 0  # If velocities are missing, set vij to 0
                
                dij = self.get_distance(self.positions[i], self.positions[j])
                
                # Check if vij is zero to avoid division by zero in delta_t calculation
                if vij == 0:
                    rlij = 0  # Default reliability value when relative velocity is zero
                else:
                    delta_t = (self.communication_range - dij) / vij  # Prediction interval
                    rlij = self.calculate_reliability(dij, delta_t, vij, self.shadow_std, self.shadow_std)
    ...
    def update_shadow(self, delta_distance_list):
        ...
        for i in range(len(delta_distance_list)):
            for j in range(len(delta_distance_list)):
    ...
    def get_path_loss(self, position_A, position_B):
        ...
        d = math.hypot(d1, d2) + 0.001  # Prevent division by zero

        d_bp = 4 * (self.h_bs - 1) * (self.h_ms - 1) * self.fc * (10 ** 9) / (3 * 10 ** 8)  # Breakpoint distance
        ...
        if min(d1, d2) < 7:  # LOS check
            ...
            self.ifLOS = False
            self.shadow_std = 4  # If Non-line of sight, increase std deviation
    ...
    def calculate_reliability(self, dij, dt, vij, sigma_vij, mu_vij):
        # Calculate the reliability parameter using the Gauss error function
        term1 = (2 * dij) / (sqrt(2) * sigma_vij * vij)
        term2 = (dt - mu_vij) / (sqrt(2) * sigma_vij * vij)
        term3 = (dt + dt - mu_vij) / (sqrt(2) * sigma_vij * vij)
        rlij = erf(term1 * term2) - erf(term1 * term3)
        return rlij
...
class Environ:
    ...
    def __init__ (self, down_lane, up_lane, left_lane, right_lane, width, height):
        ...
        self.n_RB = 20
        self.n_Veh = 60
        velocities = np.random.normal(0, 10, self.n_Veh)  # Random velocity values for vehicles
        communication_range = 100  # Example communication range in meters

        self.V2Vchannels = V2Vchannels(self.n_Veh, self.n_RB, velocities, communication_range)  # number of vehicles
