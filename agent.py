...
class Agent(BaseModel):
    ...
    def train(self):        
        ...
        for self.step in (range(0, 5000)): # need more configuration
    ...
    def play(self, n_step=100, n_episode=100, test_ep=None, render=False): 
        ...
        for game_idx in range(number_of_game): 
                ...
                for i in range(len(self.env.vehicles)): 
                    ...
                    for j in sorted_idx: 
                        ...
                        if power_selection == 0: 
                            power_select_list_0.append(time_left) 
                        elif power_selection == 1: 
                            power_select_list_1.append(time_left) 
                        elif power_selection == 2: 
                            power_select_list_2.append(time_left) 
            ...
            bins = 10 
            number_0, bin_edges = np.histogram(power_select_list_0, bins=bins) 
            number_1, bin_edges = np.histogram(power_select_list_1, bins=bins) 
            number_2, bin_edges = np.histogram(power_select_list_2, bins=bins) 

            total_counts = number_0 + number_1 + number_2 
            total_counts[total_counts == 0] = 1  # Avoid division by zero 
            p_0 = number_0 / total_counts 
            p_1 = number_1 / total_counts 
            p_2 = number_2 / total_counts 
            ...
            plot_filename = f"game_{game_idx}_power_selection.png" 
        ...
        # Additional plots after all games
